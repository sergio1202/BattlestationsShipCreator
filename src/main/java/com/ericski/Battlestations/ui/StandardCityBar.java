...
import com.ericski.Battlestations.PlanetFactory;
...
public class StandardCityBar extends JPanel
{
	private static final String TOGGLEACTION = "TOGGLEACTION";
	private static final long serialVersionUID = 1L;
	private HashMap<String, Planet> cityMap;
	private HashMap<String, JPanel> panelMap;
	PlanetCreatorPanel parentPanel;
	JButton hiderButton;
	JOutlookBar tileBar;
	...
	public StandardCityBar(final PlanetCreatorPanel cityCreatorPanel)
	{
		super(new BorderLayout());
		this.parentPanel = cityCreatorPanel;
		...
		JLabel label = new JLabel("Standard Cities");
		...
		tileBar = new JOutlookBar(JTabbedPane.RIGHT);

		List<Planet> cities = PlanetFactory.INSTANCE.getStandardTemplates();
		Collections.sort(cities);
		cityMap = new HashMap<>();
		panelMap = new HashMap<>();

		for (Planet city : cities)
		{
			...
			if (!panelMap.containsKey(species))
			{
				...
				tileBar.addTab(species, allScroller);
			}
			...
			JButton button = new JButton(city.getName(), new ImageIcon(city.generateThumbnailImage()));
		...
		modulePane.add(tileBar, BorderLayout.CENTER);
